...
export class ProvidersConfigService {
    ...
    async setConfig(app: App, provider: string, credentials: Record<string, any>) {
        const existing = await this.repo.findOne({
            where: { app: { id: app.id }, provider, active: true },
        });

        if (existing) {
            throw new Error(
                `Provider '${provider}' already has an active configuration for this app`
            );
        }
        ...
        if (config) {
            ...
            config = this.repo.create({ app, provider, credentials });
        }
